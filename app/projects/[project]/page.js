...
export default function ProjectDetailPage({ params = {} }) {
    ...
    // FIX: match by "route" instead of "slug"
    const projectDetails = projects.find((p) => p.route === project);
            ...
            <section className="relative w-full overflow-hidden">
                ...
                <div className="relative mx-auto max-w-7xl px-6 py-20 sm:px-8 md:px-10 lg:px-12 lg:py-24">
                    ...
                    >
                        <Link
                            href="/projects"
                            className="text-gray-600 flex items-center mb-4 hover:text-gray-900 underline-offset-4 hover:underline"
                        >
                            <ChevronLeft fontSize="small" /> <span>Back to Projects</span>
                        </Link>
                    ...
                    <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1fr_0.8fr]">
                        ...
                        >
                            ...
                            <p className="mt-3 text-gray-700">{projectDetails.summary}</p>

                            {projectDetails.problem && (
                                <>
                                    <h3 className="mt-6 text-sm font-semibold text-gray-900">Problem / Goal</h3>
                                    <p className="mt-1 text-gray-700">{projectDetails.problem}</p>
                                </>
                            )}
                            ...
                            {projectDetails.solution && (
                                <>
                                    ...
                                    <p className="mt-1 text-gray-700">{projectDetails.solution}</p>
                                </>
                            )}
                            ...
                            {projectDetails.highlights?.length > 0 && (
                                <>
                                    ...
                                    <ul className="mt-2 list-disc pl-5 text-gray-700">
                                        {projectDetails.highlights.map((h) => (
                                            <li key={h}>{h}</li>
                                        ))}
                                    </ul>
                                </>
                        ...
                        >
                            ...
                            {projectDetails.languages?.length > 0 && (
                                <>
                                    ...
                                    <div className="mt-3 flex flex-wrap gap-2 text-xs text-gray-700">
                                        {projectDetails.languages.map((lang) => (
                                ...
                                {projectDetails.live && (
                                    <li>
                                        <a
                                            ...
                                            aria-label={`Live site for ${projectDetails.name}`}
                                ...
                                {projectDetails.repo && (
                                    <li>
                                        <a
                                            ...
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            aria-label={`GitHub repo for ${projectDetails.name}`}
                                            className="underline-offset-4 hover:underline"
                                        >
                                            GitHub repository
                                        </a>
